...
abstract class TRecord {
	...
	*/
	...
	/**
	 * metodo load()
	 * recupera (retorna) un objeto de base de datos
	 * atraves de su ID y lo instancia en la memoria
	 * @param $id = ID del objeto
	 */
	public function load($id) {
		// instancia instruccion de SELECT
		$sql = new TSqlSelect;
		$sql->setEntity($this->getEntitiy());
		$sql->addColumn('*');

		// crea un criterio de seleccion baseado en el ID
		$criteria = new TCriteria;
		$criteria->add(new TFilter('id', '=', $id));

		// define el criterio de seleccion de datos
		$sql->setCriteria($criteria);

		// obtiene transaccion activa
		if ($conn = TTransaction::get()) {
			// crea mensaje de log y ejecuta la consulta 
			TTransaction::log($sql->getInstruction());
			$result = $conn->Query($sql->getInstruction());

			// si devuelve algun dato
			if ($result) {
	...
	/**
	 * metodo delete()
	 * excluye un objeto en la base de datos atraves de su ID
	 * @param $id = ID del objeto
	 */
	public function delete($id = NULL) {
		// el ID es el parametro o la propiedad ID
		$id = $id ? $id : $this->id;

		// instancia una instruccion de DELETE
		$sql = new TSqlDelete;
		$sql->setEntity($this->getEntity());

		// crea criterio de seleccion de datos
		$criteria = new TCriteria;
		$criteria->add(new TFilter('id', '=', $id));

		// define el criterio de seleccion baseado en el ID
		$sql->setCriteria($criteria);
